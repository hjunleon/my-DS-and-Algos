 ...
 */
long binary_search(const long list[], size_t i, size_t j, long q) {
  if (i > j) {
    return -1;
  }
  if (i == j) {
 ...
 * @return -1 if not found, the position of q in list otherwise.
 */
long search(size_t n, const long list[n], size_t i, size_t j, long q) {
  if (j < i) {
    return -1;
  }
  if (i == j) {
  ...
  if (list[i] < list[mid]) {
    ...
    return search(n, list, mid+1, j, q);
  }
  ...
  return search(n, list, i, mid-1, q);
}
...
int main() {
  ...
  long pos = search(n, list, 0, n - 1, query);
  if (pos == -1) {
